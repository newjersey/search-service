...
describe('CheckboxGroupFilter', () => {
  it('renders each option as an accessible, labeled checkbox inside a labeled group', () => {
    render(
      <CheckboxGroupFilter facetKey="status" legend="Status" options={options} selected={[]} onChange={vi.fn()} />,
    )
  ...
  it('calls onChange with the value added when an unchecked box is activated via keyboard', async () => {
    ...
    render(
      <CheckboxGroupFilter facetKey="status" legend="Status" options={options} selected={[]} onChange={onChange} />,
    )
    await user.tab()
